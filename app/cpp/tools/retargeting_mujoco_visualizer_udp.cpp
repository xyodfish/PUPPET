...
namespace puppet::tools::retargeting_mujoco_visualizer_udp_internal {
...
int main(int argc, char** argv) {
    using namespace puppet::tools::retargeting_mujoco_visualizer_udp_internal;

    std::string configPath = "config/tools/demo_single_chain_ik_visualizer_udp.yaml";
    if (argc > 1) {
        configPath = argv[1];
    }
    ...
    if (modelRaw == nullptr) {
        std::cerr << "[retargeting_mujoco_visualizer_udp] load model failed: " << modelError << std::endl;
        return 2;
    }
    ...
    if (dataRaw == nullptr) {
        std::cerr << "[retargeting_mujoco_visualizer_udp] make data failed" << std::endl;
        return 3;
    }
    std::unique_ptr<mjData, decltype(&mj_deleteData)> data(dataRaw, &mj_deleteData);

    if (!glfwInit()) {
        std::cerr << "[retargeting_mujoco_visualizer_udp] glfw init failed" << std::endl;
        return 4;
    }
    GLFWwindow* window = glfwCreateWindow(cfg.width, cfg.height, "PUPPET Viewer UDP", nullptr, nullptr);
    if (window == nullptr) {
        std::cerr << "[retargeting_mujoco_visualizer_udp] create window failed" << std::endl;
        glfwTerminate();
        return 5;
    }
    ...
    if (sockFd < 0) {
        std::cerr << "[retargeting_mujoco_visualizer_udp] create socket failed: " << std::strerror(errno) << std::endl;
        return 6;
    }
    int enableReuse = 1;
    ...
    if (inet_pton(AF_INET, cfg.controlIntentBindIp.c_str(), &bindAddr.sin_addr) != 1) {
        std::cerr << "[retargeting_mujoco_visualizer_udp] invalid bind ip: " << cfg.controlIntentBindIp << std::endl;
        close(sockFd);
        return 7;
    }
    if (bind(sockFd, reinterpret_cast<const sockaddr*>(&bindAddr), sizeof(bindAddr)) != 0) {
        std::cerr << "[retargeting_mujoco_visualizer_udp] bind failed: " << std::strerror(errno) << std::endl;
        close(sockFd);
        return 8;
    }
    std::cout << "[retargeting_mujoco_visualizer_udp] listening on " << cfg.controlIntentBindIp << ":" << cfg.controlIntentBindPort
              << std::endl;
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        const ssize_t rc  = recvfrom(sockFd, recvBuf.data(), recvBuf.size(), MSG_DONTWAIT, reinterpret_cast<sockaddr*>(&peer), &peerLen);
        if (rc > 0) {
            ::puppet::puppet_proto::ControlIntent msg;
            if (msg.ParseFromArray(recvBuf.data(), static_cast<int>(rc))) {
                cache.update(msg);
            }
        }
        ...
        if (cfg.printStat) {
            ...
            if (messageCount != lastPrintCount) {
                ...
                std::cout << "[retargeting_mujoco_visualizer_udp] msg_count=" << messageCount << " last_seq=" << lastSeq
