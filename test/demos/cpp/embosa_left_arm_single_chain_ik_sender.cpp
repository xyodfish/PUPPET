...
namespace {
    ...
    bool initLeftArmFkContext(ArmFkContext* ctx) {
        if (ctx == nullptr) {
            return false;
        }
        ctx->jointNames = {
        ...
        ctx->solver =
            std::make_unique<TRAC_IK::TRAC_IK>("torso_link", "left_wrist_yaw_link", "assets/unitree_g1/g1_custom_collision_29dof.urdf",
                                               10000, 0.003, 1e-5, false, false, false, TRAC_IK::Speed);
        return ctx->solver->getKDLChain(ctx->chain);
    }

    std::vector<double> makeLeftArmJointPositions(double t_sec) {
        return {
    ...
    void fillHeaderAndContext(PrimitiveFramePb* frame, uint64_t sequence_id, const char* mode, const char* pipelineId) {
        ...
        frame->mutable_context()->set_source_id("sourceB");
    ...
    std::shared_ptr<PrimitiveFramePb> buildSingleChainIkFrame(uint64_t sequence_id, const ArmFkContext& ctx, double t_sec) {
        ...
        const auto jointPositions = makeLeftArmJointPositions(t_sec);
        const auto eeFrame        = computeEndEffectorFrame(ctx, jointPositions);

        auto* pose = frame->add_poses();
        pose->mutable_meta()->set_name("left_wrist_pose_demo");
        pose->mutable_meta()->set_entity("left_wrist");
        ...
        joint_state->mutable_meta()->set_name("left_arm_seed_state");
        joint_state->mutable_meta()->set_entity("left_arm");
        joint_state->mutable_meta()->set_body_group("left_arm");
        ...
        (*frame->mutable_tags())["source"]   = "sourceB";
    ...
    std::shared_ptr<PrimitiveFramePb> buildDirectPassFrame(uint64_t sequence_id, const ArmFkContext& ctx, double t_sec) {
        ...
        joint_cmd->mutable_meta()->set_name("left_arm_joint_command_demo");
        joint_cmd->mutable_meta()->set_entity("left_arm");
        joint_cmd->mutable_meta()->set_body_group("left_arm");
...
int main() {
    ...
    if (!initLeftArmFkContext(&fkContext)) {
        std::cerr << "[left_arm_single_chain_ik_sender] failed to initialize left-arm FK context\n";
        return 1;
    }

    galbot::embosa::EmbosaInit();
    auto node = galbot::embosa::CreateNode("puppet_left_arm_single_chain_ik_sender");
    if (node == nullptr) {
        std::cerr << "[left_arm_single_chain_ik_sender] failed to create embosa node\n";
        return 1;
    }

    auto writer = node->CreateWriter<PrimitiveFramePb>("puppet_demo/primitive_frame");
    if (writer == nullptr) {
        std::cerr << "[left_arm_single_chain_ik_sender] failed to create writer\n";
        return 1;
    }
    ...
    while (galbot::embosa::OK()) {
        ...
        if ((seq % 25U) == 0U) {
            std::cout << "[left_arm_single_chain_ik_sender] seq=" << seq << " strategy=" << frame->context().pipeline_id();
            if (frame->joint_commands_size() > 0) {
                std::cout << " left_elbow=" << frame->joint_commands(0).position(3);
            } else if (frame->poses_size() > 0) {
                std::cout << " left_wrist_x=" << frame->poses(0).pose().position().x();
            }
            std::cout << "\n";
        }
