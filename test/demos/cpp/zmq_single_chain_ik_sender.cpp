...
namespace puppet::test::zmq_single_chain_ik_sender_detail {
    ...
    PrimitiveFramePb buildFrame(uint64_t sequence_id, double t_sec) {
    ...
    bool publishFrame(void* publisher, const std::string& topic, const PrimitiveFramePb& frame, std::string* error) {
        std::string payload(frame.ByteSizeLong(), '\0');
        if (!frame.SerializeToArray(payload.data(), static_cast<int>(payload.size()))) {
            *error = "serialize PrimitiveFrame failed";
            return false;
        }

        if (zmq_send(publisher, topic.data(), topic.size(), ZMQ_SNDMORE) < 0) {
            *error = std::string("send topic failed: ") + zmq_strerror(errno);
            return false;
        }
        if (zmq_send(publisher, payload.data(), payload.size(), 0) < 0) {
            *error = std::string("send payload failed: ") + zmq_strerror(errno);
            return false;
        }
        return true;
    }
...
int main(int argc, char** argv) {
    using namespace puppet::test::zmq_single_chain_ik_sender_detail;

    std::signal(SIGINT, signalHandler);
    std::signal(SIGTERM, signalHandler);

    std::string endpoint = "tcp://127.0.0.1:16661";
    std::string topic    = "puppet_demo/primitive_frame";
    if (argc > 1) {
        endpoint = argv[1];
    }
    if (argc > 2) {
    ...
    if (context == nullptr) {
        std::cerr << "[zmq_single_chain_ik_sender] create context failed: " << zmq_strerror(errno) << "\n";
        return 1;
    }
    void* publisher = zmq_socket(context, ZMQ_PUB);
    if (publisher == nullptr) {
        std::cerr << "[zmq_single_chain_ik_sender] create publisher failed: " << zmq_strerror(errno) << "\n";
        zmq_ctx_term(context);
        return 1;
    }
    if (zmq_bind(publisher, endpoint.c_str()) != 0) {
        std::cerr << "[zmq_single_chain_ik_sender] bind failed endpoint=" << endpoint << " error=" << zmq_strerror(errno) << "\n";
        zmq_close(publisher);
        zmq_ctx_term(context);
        return 1;
    }
    ...
    while (!gStopRequested.load()) {
        ...
        if (!publishFrame(publisher, topic, frame, &error)) {
            std::cerr << "[zmq_single_chain_ik_sender] publish failed: " << error << "\n";
            break;
        }

        if ((seq % 25U) == 0U) {
            std::cout << "[zmq_single_chain_ik_sender] seq=" << seq << " right=(" << frame.poses(0).pose().position().x() << ", "
